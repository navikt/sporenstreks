...
internal class ExcelParserTest {
    ...
    fun `Parseren skal gi feil på riktig rad og kolonne`() = suspendableTest {
        ...
        assertThat(rowErrors[12]?.size).isEqualTo(2)
        assertThat(rowErrors[12]?.get(0)?.column).isEqualTo("Arbeidsgiverperioden (fom+tom)")
        assertThat(rowErrors[12]?.get(1)?.column).isEqualTo("Fødselsnummer")

        assertThat(rowErrors[13]?.size).isEqualTo(2)
        assertThat(rowErrors[13]?.get(0)?.column).isEqualTo("Arbeidsgiverperioden (fom+tom)")
        assertThat(rowErrors[13]?.get(1)?.column).isEqualTo("Virksomhetsnummer")
