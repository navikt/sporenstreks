...
const val FEILEDE_KVITTERINGER_TO_PROCESS_LIMIT = 250


class ProcessFeiledeKvitteringerJob(
    ...
    override fun doJob() {
        if (runBlocking { leaderElectionConsumer.isLeader() })
            return
        mutualLock.lock()
        db.getByStatus(KvitteringStatus.FEILET, FEILEDE_KVITTERINGER_TO_PROCESS_LIMIT)
                .forEach {
